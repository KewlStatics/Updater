...
let shop = [
	...
	['customcolor', 'buys a customcolor for ur name', 25],
	...
	['Userlist Icon', 'Purchases a userlist icon of your choice, PM an Admin with the icon you want(Size must be 32x32, 3 rooms maximum, will take time to appear).', 350],
...
function alertStaff(msg) {
	Users.users.forEach(function (user) {
		if (user.isStaff) {
			user.send('|pm|~Mists Shopkeeper|' + user.getIdentity() + '|' + msg);
		}
	});
}
...
function handleBoughtItem(item, user, cost) {
	if (item === 'symbol') {
		...
		/*if (item === 'customcolor') {
			user.sendCustomcolor = true;
			this.sendReply("|raw|<div class='brodcast-green'>You have purchased an avatar, use /sendcolor [color hex] to let the staff know what color you want.</div>");
		*/
...
exports.commands = {
	/*sendcolor: function (target, room, user) {
		if (!user.sendcustomcolor) return this.sendReply("You need to buy this item from the shop.");
		if (!target) return this.parse('/help sendcolor');
		let msg = '**' + user + " has purchased an Customcolor and wants" + '** ' + target + ' **' + "as their color." + '**';
		Rooms.rooms.staff.add('|c|~Shop Alert|' + msg);
		Rooms.rooms.staff.update();
		alertStaff(msg);
		user.sendAvatar = false;
	},
	sendcolorhelp: ["/sendcolor [color hex] - If you have purchased an color, use /sencolor [color hex] to let the staff know what color you want."],
*/
	balance: 'wallet',
	bal: 'wallet',
	atm: 'wallet',
	...
	givemoney: function (target, room, user) {
		if (!(user.userid === 'arkanox' || user.userid === 'rambo')) return this.sendReply("You dont have auth to give money");
		...
		//Users.get(username).popup('<b><font color="' + color(user.name) + '">' + Tools.escapeHTML(user.name) + '</font></b>has  give you'+ amount + '. you now have'+ total + '.');
